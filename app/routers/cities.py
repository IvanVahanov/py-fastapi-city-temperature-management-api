...
def get_cities(skip: int = 0, limit: int = 10, db: Session = Depends(get_db)) -> list[schemas.City] :
    return crud.get_cities(db=db, skip=skip, limit=limit)

@router.get("/cities/{city_id}", response_model=schemas.City)
def get_city(city_id: int, db: Session = Depends(get_db)) -> schemas.City:
...
def update_city(city_id: int, city_update: schemas.CityUpdate, db: Session = Depends(get_db)) -> schemas.City:
